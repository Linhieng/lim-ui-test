...
// TODO: 是否可以自己实现 jsonpointer.get 模块，因为 npm 提示 jsonpointer 模块有一个中度严重安全漏洞。而且目前这个工具库中只用到了 jsonpointer.get。查看模块仓库源码，发现源码也很简单，完全可以自己实现。
// TODO: 阅读该文件代码
/*
相关说明：
resolveSchema 函数用于解析 schema，其中需要注意的有三个属性：
    $ref
        该属性表示引用其他的 schema 定义，所以在 resolveSchema 中需要解析出 $ref 所引用的 schema 是什么
    dependencies
        简单的理解就是，该属性用来实现这么一种场景：比如 select 的不同值，将会匹配不同的 schema 定义。
        就像是填写表单时，如果你勾选上了“地址”，那么表单中就需要有一个地址信息。如果没有勾选，则不应该出现地址信息这样
    allOf
        TODO: 解释 allOf

比如下面这个案例，当 select 的值为 true 时，要求有一个 name；为false 时，要求有一个 number 属性。具体的原理是根据 oneOf 来设置的
{
  "properties": {
    "select": {
      "type": "boolean"
  ...
  "dependencies": {
    "select": {
      "oneOf": [ // oneOf 数组表示只会使用这里面的一个对象（schema），即 select 为 true 时，将生成 select 和 name，select 为 false 时，将生成 select 和 age
        {
          "properties": {
            "select": {
              "const": true
...
function processDependencies(
    ...
    for (const dependencyKey in dependencies) {
        ...
        if (Array.isArray(dependencyValue)) {
            resolvedSchema = withDependentProperties(
                resolvedSchema,
                dependencyValue
            )
